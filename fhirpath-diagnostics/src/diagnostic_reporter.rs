...
use crate::{EnhancedDiagnostic, Severity, DiagnosticCode, SourceLocation};
...
impl DiagnosticReporter {
    ...
    fn generate_summary(&self) -> DiagnosticSummary {
        ...
        summary.common_codes = code_vec.into_iter()
            .take(5)
    ...
    fn group_diagnostics(&self) -> GroupedDiagnostics {
        ...
        for (_index, diagnostic) in self.diagnostics.iter().enumerate() {
            // Group by severity
            grouped.by_severity
    ...
    fn detect_error_patterns(&self, grouped: &GroupedDiagnostics) -> Vec<ErrorPattern> {
        ...
        if let Some(undefined_vars) = grouped.by_code.get("E202") {
            if undefined_vars.len() > 1 {
                patterns.push(ErrorPattern {
                    ...
                    confidence: 0.9,
                    suggested_fix: "Define variables before use or check variable names for typos".to_string(),
                });
            }
        }
        
        // Pattern: Syntax error cascade
        let syntax_errors: Vec<_> = grouped.by_code.keys()
        ...
        if syntax_errors.len() > 2 {
            let matching_diagnostics = syntax_errors
                .iter()
                .filter_map(|code| grouped.by_code.get(code.as_str())).flat_map(|v| v)
                .enumerate()
                .map(|(i, _)| i)
                .collect();
            ...
            patterns.push(ErrorPattern {
                ...
                description: "Multiple syntax errors, likely caused by a single mistake".to_string(),
                matching_diagnostics,
                confidence: 0.8,
                suggested_fix: "Fix the first syntax error, others may resolve automatically".to_string(),
            });
        }
        ...
        if let Some(type_errors) = grouped.by_code.get("E100") {
            if type_errors.len() > 1 {
                patterns.push(ErrorPattern {
                    ...
                    confidence: 0.7,
                    suggested_fix: "Check the types of variables and function parameters".to_string(),
                });
            }
        }
        ...
        for (primary_idx, primary_diagnostic) in self.diagnostics.iter().enumerate() {
            ...
            match &primary_diagnostic.diagnostic.code {
                DiagnosticCode::UnclosedString => {
                    ...
                    for (idx, diagnostic) in self.diagnostics.iter().enumerate().skip(primary_idx + 1) {
                        if matches!(diagnostic.diagnostic.code, 
                    ...
                    if !secondary_indices.is_empty() {
                        root_causes.push(RootCause {
                            description: "Unclosed string literal causing parser confusion".to_string(),
                ...
                DiagnosticCode::ExpectedToken(token) if token == ")" => {
                    ...
                    if !secondary_indices.is_empty() {
                        root_causes.push(RootCause {
                            description: "Missing closing parenthesis causing parse errors".to_string(),
                            primary_diagnostic: primary_idx,
                            secondary_diagnostics: secondary_indices,
                            confidence: 0.8,
                            fix_priority: 1,
    ...
    fn find_diagnostic_indices(&self, diagnostics: &[EnhancedDiagnostic]) -> Vec<usize> {
        diagnostics
            ...
            .enumerate()
            .filter_map(|(i, d)| {
                self.diagnostics.iter().position(|our_d| our_d == d).map(|_| i)
            })
            .collect()
    }
    ...
    fn could_be_cascading_error(&self, initial: &EnhancedDiagnostic, potential_cascade: &EnhancedDiagnostic) -> bool {
    ...
    fn calculate_analysis_confidence(&self, analysis: &DiagnosticAnalysis) -> f32 {
        ...
        let pattern_confidence = if analysis.error_patterns.is_empty() {
            ...
            analysis.error_patterns.iter().map(|p| p.confidence).sum::<f32>() / analysis.error_patterns.len() as f32
        };
        
        let root_cause_confidence = if analysis.root_causes.is_empty() {
            0.5
        } else {
            analysis.root_causes.iter().map(|r| r.confidence).sum::<f32>() / analysis.root_causes.len() as f32
        };
    ...
    pub fn errors(&self) -> Vec<&EnhancedDiagnostic> {
        self.diagnostics.iter().filter(|d| d.diagnostic.is_error()).collect()
    }
    
    /// Get all warning diagnostics
    pub fn warnings(&self) -> Vec<&EnhancedDiagnostic> {
        self.diagnostics.iter().filter(|d| d.diagnostic.is_warning()).collect()
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if !self.analysis.error_patterns.is_empty() {
            ...
            for pattern in &self.analysis.error_patterns {
                writeln!(f, "  • {} (confidence: {:.0}%)", pattern.name, pattern.confidence * 100.0)?;
        ...
        if !self.analysis.root_causes.is_empty() {
            ...
            for root_cause in &self.analysis.root_causes {
                writeln!(f, "  • {} (confidence: {:.0}%)", root_cause.description, root_cause.confidence * 100.0)?;
            }
            writeln!(f)?;
        }
        ...
        if !self.suggested_workflow.is_empty() {
            ...
            for step in &self.suggested_workflow {
                writeln!(f, "  {}. {} (effort: {}/5)", step.step_number, step.title, step.effort)?;
...
mod tests {
    ...
    fn create_test_diagnostic(severity: Severity, code: DiagnosticCode, message: &str) -> EnhancedDiagnostic {
    ...
    fn test_report_generation() {
        ...
        reporter.add_diagnostic(create_test_diagnostic(
            Severity::Error, DiagnosticCode::UnknownFunction, "Unknown function 'test'"
        ));
        reporter.add_diagnostic(create_test_diagnostic(
            Severity::Warning, DiagnosticCode::TypeMismatch { 
                expected: "string".to_string(), 
                actual: "integer".to_string() 
            }, "Type mismatch"
        ));
        reporter.add_diagnostic(create_test_diagnostic(
            Severity::Error, DiagnosticCode::UndefinedVariable, "Undefined variable '$test'"
        ));
    ...
    fn test_root_cause_analysis() {
        ...
        reporter.add_diagnostic(create_test_diagnostic(
            Severity::Error, DiagnosticCode::UnclosedString, "Unclosed string literal"
        ));
        reporter.add_diagnostic(create_test_diagnostic(
            Severity::Error, DiagnosticCode::UnexpectedToken, "Unexpected token"
        ));
