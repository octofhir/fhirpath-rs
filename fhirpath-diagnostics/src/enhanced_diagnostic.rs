...
pub struct SuggestionGenerator {
    ...
    pattern_explanations: HashMap<String, String>,
}
...
impl EnhancedDiagnostic {
    ...
    fn generate_unknown_function_help(&mut self, generator: &SuggestionGenerator) {
        self.context.push("FHIRPath functions must be defined in the function registry.".to_string());
        self.context.push("Check for typos in the function name or verify the function is available.".to_string());
        ...
        if let Some(func_name) = self.extract_function_name_from_message() {
            ...
            if let Some(correction) = generator.get_typo_correction(&func_name) {
                self.quick_fixes.push(QuickFix {
                    title: format!("Replace '{}' with '{}'", func_name, correction),
    ...
    fn generate_expected_token_help(&mut self, _generator: &SuggestionGenerator, expected: &str) {
        self.context.push(format!("The parser expected to find '{}' at this position.", expected));
        
        match expected {
            ")" => {
                self.context.push("Check for unmatched parentheses in your expression.".to_string());
                self.smart_suggestions.push(SmartSuggestion {
            ...
            "]" => {
                self.context.push("Check for unmatched brackets in array indexing.".to_string());
                self.smart_suggestions.push(SmartSuggestion {
            ...
            "." => {
                self.context.push("Property access requires a dot (.) before the property name.".to_string());
                self.smart_suggestions.push(SmartSuggestion {
    ...
    fn generate_type_mismatch_help(&mut self, _generator: &SuggestionGenerator, expected: &str, actual: &str) {
        self.context.push(format!("Expected type '{}' but found '{}'.", expected, actual));
        self.context.push("Type mismatches often occur when using incorrect operators or function arguments.".to_string());
        ...
        match (expected, actual) {
            ...
            ("boolean", _) => {
                self.smart_suggestions.push(SmartSuggestion {
                    ...
                    confidence: 0.7,
                    rationale: "Boolean expressions use comparison or logical operators".to_string(),
            ...
            _ => {
                self.smart_suggestions.push(SmartSuggestion {
                    message: format!("Check if type conversion from {} to {} is needed", actual, expected),
                    confidence: 0.6,
    ...
    fn generate_arity_help(&mut self, generator: &SuggestionGenerator) {
        self.context.push("The function was called with the wrong number of arguments.".to_string());
        self.context.push("Check the function signature to see the required parameters.".to_string());
        
        // Extract function name and suggest correct signature
        if let Some(func_name) = self.extract_function_name_from_message() {
            if let Some(signature) = generator.get_function_signature(&func_name) {
                let param_list: Vec<String> = signature.parameters
                    .iter()
                    .map(|p| if p.optional {
                        format!("[{}]", p.name)
                    } else {
                ...
                self.smart_suggestions.push(SmartSuggestion {
                    message: format!("{}({}) - {}", signature.name, param_list.join(", "), signature.description),
                    confidence: 0.9,
    ...
    fn generate_undefined_variable_help(&mut self, generator: &SuggestionGenerator) {
        self.context.push("Variables in FHIRPath start with '$' and must be defined in the current scope.".to_string());
        self.context.push("Common variables include $this, $context, and $resource.".to_string());
            ...
            for similar in similar_vars {
                if generator.is_similar_string(&var_name, similar) {
                    self.smart_suggestions.push(SmartSuggestion {
                        ...
                        example: Some(format!("{}.value", similar)),
                        category: SuggestionCategory::CommonMistake,
                    });
                }
            }
    ...
    fn generate_unexpected_token_help(&mut self, _generator: &SuggestionGenerator) {
        self.context.push("The parser encountered a token it didn't expect at this position.".to_string());
        self.context.push("This often indicates a syntax error or typo.".to_string());
    ...
    fn generate_generic_help(&mut self, _generator: &SuggestionGenerator) {
        self.context.push("Refer to the FHIRPath specification for detailed syntax rules.".to_string());
...
impl SuggestionGenerator {
    pub fn new() -> Self {
        let mut generator = Self {
            ...
            pattern_explanations: HashMap::new(),
        };
    ...
    fn initialize_common_corrections(&mut self) {
        ...
        self.typo_corrections.insert("lenght".to_string(), vec!["length".to_string()]);
        self.typo_corrections.insert("exsits".to_string(), vec!["exists".to_string()]);
        self.typo_corrections.insert("whre".to_string(), vec!["where".to_string()]);
        self.typo_corrections.insert("slect".to_string(), vec!["select".to_string()]);
        self.typo_corrections.insert("frist".to_string(), vec!["first".to_string()]);
        self.typo_corrections.insert("lsat".to_string(), vec!["last".to_string()]);
        self.typo_corrections.insert("singel".to_string(), vec!["single".to_string()]);
        self.typo_corrections.insert("conatin".to_string(), vec!["contains".to_string()]);
        self.typo_corrections.insert("startsWtih".to_string(), vec!["startsWith".to_string()]);
        self.typo_corrections.insert("endsWtih".to_string(), vec!["endsWith".to_string()]);
    }
    ...
    fn initialize_function_signatures(&mut self) {
        ...
        self.function_signatures.insert("exists".to_string(), FunctionSignature {
            ...
            description: "Returns true if any items exist (optionally matching criteria)".to_string(),
            examples: vec![
    ...
    pub fn get_typo_correction(&self, name: &str) -> Option<&String> {
        self.typo_corrections.get(name).and_then(|corrections| corrections.first())
    }
...
mod tests {
    ...
    fn test_enhanced_diagnostic_from_type_mismatch() {
        ...
        let has_tostring_suggestion = enhanced.smart_suggestions
            .iter()
