...
impl ExtensionMetadata {
    ...
    pub fn validate_namespace(&self) -> Result<(), String> {
        ...
        if !namespace.chars().next().unwrap_or(' ').is_ascii_alphabetic() {
        ...
        if !namespace.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_') {
            return Err("Namespace can only contain letters, digits, hyphens, and underscores".to_string());
        }
        
        // Reserved namespaces
        const RESERVED: &[&str] = &["core", "std", "builtin", "system"];
        if RESERVED.contains(&namespace.as_str()) {
            return Err(format!("Namespace '{}' is reserved", namespace));
        }
...
mod tests {
    ...
    fn test_dependency_creation() {
        ...
        let optional_dep = ExtensionDependency::optional("optional").with_version_range(
