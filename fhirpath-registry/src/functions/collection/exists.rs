...
use crate::function::{
    ...
    LambdaEvaluationContext, LambdaFunction,
};
...
impl LambdaFunction for ExistsFunction {
    fn evaluate_with_lambda(
        ...
        if args.is_empty() {
            ...
            return Ok(FhirPathValue::collection(vec![FhirPathValue::Boolean(exists)]));
        }
        ...
        let items = match &context.context.input {
            ...
            FhirPathValue::Empty => return Ok(FhirPathValue::collection(vec![FhirPathValue::Boolean(false)])), // Empty collection
