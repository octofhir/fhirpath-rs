...
impl FhirPathFunction for RepeatFunction {
    ...
    fn evaluate(
        ...
        if matches!(expression_arg, FhirPathValue::String(_) | FhirPathValue::Integer(_) | FhirPathValue::Boolean(_) | FhirPathValue::Decimal(_)) {
...
fn value_to_fhir_path_value(value: &serde_json::Value) -> FhirPathValue {
    use fhirpath_model::FhirResource;
    ...
    match value {
        ...
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                FhirPathValue::Decimal(rust_decimal::Decimal::from_f64_retain(f).unwrap_or_default())
            } else {
                FhirPathValue::Empty
            }
        }
