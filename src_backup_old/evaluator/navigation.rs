...
impl TypeAwareNavigator {
                ...
                Ok(validation) => {
                    if !validation.is_valid {
                        return Err(EvaluationError::PropertyNotFound {
                            ...
                            suggestions: validation.messages.clone(),
                        });
                    }
        ...
        match value {
            ...
            serde_json::Value::Bool(b) => {
                if is_fhir_context {
                    ...
                    FhirPathValue::Resource(Arc::new(crate::model::FhirResource::from_json(
                        value.clone(),
                    )))
                } else {
            ...
            serde_json::Value::Number(n) => {
                if is_fhir_context {
                    ...
                    if let Ok(decimal) = rust_decimal::Decimal::try_from(f) {
                        ...
                        FhirPathValue::JsonValue(crate::model::json_arc::ArcJsonValue::new(
                            value.clone(),
                        ))
                    }
                } else {
            ...
            serde_json::Value::Array(arr) => {
                let collection_items = arr
                    ...
                    .collect();
                FhirPathValue::Collection(crate::model::Collection::from_vec(collection_items))
            }
    ...
    fn json_value_to_fhirpath_value(&self, value: &serde_json::Value) -> FhirPathValue {
        match value {
            ...
            serde_json::Value::Object(_) => {
                FhirPathValue::JsonValue(crate::model::json_arc::ArcJsonValue::new(value.clone()))
            }
        }
    }
...
mod tests {
    ...
    async fn test_sync_navigation() {
        ...
        let input = FhirPathValue::JsonValue(crate::model::json_arc::ArcJsonValue::new(json_obj));
