...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = engine.evaluate("Patient.name.where(use = 'official').family", patient_json.clone())?;
    ...
    print_result("Patient.name.where(use = 'official').given.first() + ' ' + Patient.name.where(use = 'official').family", &result);
    ...
    let result = engine.evaluate("Patient.contact.telecom.where(system = 'email').exists()", patient_json.clone())?;
    print_result("Patient.contact.telecom.where(system = 'email').exists()", &result);

    // Example 10: Complex expressions with select
    println!("🔟 Complex Expressions with Select");
    println!("------------------------------------");
    
    let result = engine.evaluate("Patient.name.select(given.first() + ' ' + family)", patient_json.clone())?;
...
fn print_result(expression: &str, result: &FhirPathValue) {
    ...
    match result {
        ...
        FhirPathValue::Collection(items) if items.is_empty() => println!("Result: (empty collection)"),
