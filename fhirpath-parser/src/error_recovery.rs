...
use fhirpath_diagnostics::{Diagnostic, DiagnosticCode, EnhancedDiagnostic, Severity, SourceLocation};
...
pub fn analyze_recovery_potential(input: &str) -> RecoveryAnalysis {
    ...
    if depth != 0 {
        analysis.error_prone_constructs.push("Unmatched delimiters".to_string());
        analysis.suggested_strategy = RecoveryStrategy::Aggressive;
    }
    
    if has_complex_nesting {
        analysis.error_prone_constructs.push("Complex nesting detected".to_string());
