...
impl FunctionAnalyzer {
        ...
        if let Some(registry_signature) = self.registry.get_function_signature(name).await {
            ...
            let analyzer_signature = FunctionSignature {
                ...
                description: format!(
                    "{}({}) -> {}",
                    name,
                    self.format_parameters(&registry_signature.parameters),
                    self.format_return_type(&registry_signature.return_type)
                ),
            };
    ...
    fn is_similar_function_name(&self, input: &str, candidate: &str) -> bool {
        ...
        if input_lower.len() >= 2 && candidate_lower.len() >= 2 {
            ...
            if input_lower.starts_with(&candidate_lower)
    ...
    fn convert_parameter_type(
        &self,
        param_type: &octofhir_fhirpath_registry::signature::ParameterType,
    ) -> TypeInfo {
        use octofhir_fhirpath_registry::signature::ParameterType;
    ...
    fn convert_value_type(
        &self,
        value_type: &octofhir_fhirpath_registry::signature::ValueType,
    ) -> TypeInfo {
    ...
    fn format_parameters(
        &self,
        parameters: &[octofhir_fhirpath_registry::signature::ParameterType],
    ) -> String {
        parameters
            .iter()
    ...
    fn format_return_type(
        &self,
        return_type: &octofhir_fhirpath_registry::signature::ValueType,
    ) -> String {
        use octofhir_fhirpath_registry::signature::ValueType;
...
mod tests {
    ...
    async fn test_function_analyzer_unknown_function() {
        ...
        assert!(!analysis.validation_errors.is_empty());
        assert_eq!(
            analysis.validation_errors[0].error_type,
