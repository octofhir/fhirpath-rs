...
//! Type coercion utilities for FHIRPath values
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CoercionError::InvalidFormat { value, target_type } => {
                write!(f, "Invalid format '{value}' for type {target_type}")
            }
            CoercionError::MultipleItems => {
                write!(
                    f,
                    "Cannot coerce collection with multiple items to single value"
                )
            }
...
impl TypeCoercion {
    ...
    pub fn coerce_to_decimal(value: &FhirPathValue) -> CoercionResult<FhirPathValue> {
        match value {
            ...
            FhirPathValue::Boolean(b) => Ok(FhirPathValue::Decimal(if *b {
                Decimal::ONE
            } else {
                Decimal::ZERO
            })),
    ...
    pub fn coerce_to_string(value: &FhirPathValue) -> CoercionResult<FhirPathValue> {
        if let Some(string_val) = value.to_string_value() {
            Ok(FhirPathValue::String(string_val.into()))
        } else {
            match value {
                FhirPathValue::Collection(items) => {
                    if items.len() > 1 {
    ...
    pub fn find_common_type(values: &[FhirPathValue]) -> TypeInfo {
        ...
        if values
            .iter()
            .all(|v| matches!(v, FhirPathValue::Integer(_) | FhirPathValue::Decimal(_)))
        {
        ...
        let unique_types: std::collections::HashSet<_> =
            values.iter().map(|v| v.to_type_info()).collect();
...
mod tests {
    ...
    fn test_boolean_coercion() {
        ...
        // String to boolean
        assert_eq!(
            TypeCoercion::coerce_to_boolean(&FhirPathValue::String("true".to_string().into()))
                .unwrap(),
            FhirPathValue::Boolean(true)
        );
        assert_eq!(
            TypeCoercion::coerce_to_boolean(&FhirPathValue::String("false".to_string().into()))
                .unwrap(),
            FhirPathValue::Boolean(false)
        );
        ...
        assert!(
            TypeCoercion::coerce_to_boolean(&FhirPathValue::String("invalid".to_string().into()))
                .is_err()
        );
    }
    ...
    fn test_integer_coercion() {
        ...
        assert!(
            TypeCoercion::coerce_to_integer(&FhirPathValue::Decimal(Decimal::new(425, 1))).is_err()
        );

        // String to integer
        assert_eq!(
            TypeCoercion::coerce_to_integer(&FhirPathValue::String("42".to_string().into()))
                .unwrap(),
            FhirPathValue::Integer(42)
        );
        ...
        assert!(
            TypeCoercion::coerce_to_integer(&FhirPathValue::String("42.5".to_string().into()))
                .is_err()
        );
    }
    ...
    fn test_common_type_detection() {
        ...
        assert_eq!(
            TypeCoercion::find_common_type(&mixed_numeric),
            TypeInfo::Decimal
        );
    }
}
