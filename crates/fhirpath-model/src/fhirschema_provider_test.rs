...
mod tests {
    ...
    async fn test_schema_based_choice_property_detection() {
        let provider = FhirSchemaModelProvider::new().await.expect("Failed to create provider");
        ...
        assert!(is_choice, "Observation.value should be detected as a choice property");
        ...
        assert!(is_choice_patient, "Patient.deceased should be detected as a choice property");
        ...
        assert!(!is_not_choice, "Patient.name should not be a choice property");
    }
    ...
    async fn test_choice_variant_retrieval() {
        ...
        println!("Found {} choice variants for Observation.value", variants.len());
        assert!(!variants.is_empty(), "Should find choice variants for Observation.value");
    ...
    async fn test_choice_property_resolution() {
        ...
        let observation_value = FhirPathValue::JsonValue(JsonValue::from(observation_with_quantity));
        let resolved = provider.resolve_choice_property("Observation", "value", &observation_value).await;
        ...
        let observation_string_value = FhirPathValue::JsonValue(JsonValue::from(observation_with_string));
        let resolved_string = provider.resolve_choice_property("Observation", "value", &observation_string_value).await;
    ...
    async fn test_get_choice_base_property() {
        ...
        let base = provider.get_choice_base_property("Observation", "valueQuantity").await;
        println!("Base property for valueQuantity: {:?}", base);
        assert_eq!(base, Some("value".to_string()));

        let base_deceased = provider.get_choice_base_property("Patient", "deceasedBoolean").await;
