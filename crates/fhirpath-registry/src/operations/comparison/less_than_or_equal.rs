...
impl Default for LessThanOrEqualOperation {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    pub fn compare_less_or_equal(
        ...
        match (left, right) {
            ...
            (FhirPathValue::Date(date), FhirPathValue::DateTime(datetime)) => {
                ...
                // This means: is there ANY moment in the date that is <= datetime?
            ...
            (FhirPathValue::DateTime(datetime), FhirPathValue::Date(date)) => {
                // For DateTime <= Date, use high boundary of Date (end of day)
                // This means: is the datetime <= ANY moment in the date?
                ...
                Ok(Some(*datetime <= date_high_boundary))
            }
...
impl FhirPathOperation for LessThanOrEqualOperation {
    ...
    fn metadata(&self) -> &OperationMetadata {
        static METADATA: std::sync::LazyLock<OperationMetadata> =
            std::sync::LazyLock::new(LessThanOrEqualOperation::create_metadata);
        &METADATA
    }
