...
#[derive(Debug, Clone)]
pub struct LessThanOrEqualOperation;

impl LessThanOrEqualOperation {
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn compare_less_or_equal(left: &FhirPathValue, right: &FhirPathValue) -> Result<Option<bool>> {
        match (left, right) {
            ...
            (FhirPathValue::Collection(a), FhirPathValue::Collection(b)) => {
                if a.len() == 1 && b.len() == 1 {
                    Self::compare_less_or_equal(a.get(0).unwrap(), b.get(0).unwrap())
                } else {
...
#[async_trait]
impl FhirPathOperation for LessThanOrEqualOperation {
    ...
    fn metadata(&self) -> &OperationMetadata {
        static METADATA: std::sync::LazyLock<OperationMetadata> = std::sync::LazyLock::new(|| {
            LessThanOrEqualOperation::create_metadata()
        });
        &METADATA
    }
    ...
    async fn evaluate(&self, args: &[FhirPathValue], _context: &EvaluationContext) -> Result<FhirPathValue> {
        ...
        binary_operator_utils::evaluate_binary_operator_optional(&args[0], &args[1], Self::compare_less_or_equal)
    }
    ...
    fn try_evaluate_sync(&self, args: &[FhirPathValue], _context: &EvaluationContext) -> Option<Result<FhirPathValue>> {
        ...
        Some(binary_operator_utils::evaluate_binary_operator_optional(&args[0], &args[1], Self::compare_less_or_equal))
    }
...
mod tests {
    ...
    async fn test_less_than_or_equal_integers() {
        let op = LessThanOrEqualOperation::new();
    ...
    #[tokio::test]
    async fn test_less_than_or_equal_mixed_numeric() {
        let op = LessThanOrEqualOperation::new();
        let ctx = create_test_context();
    ...
    #[tokio::test]
    async fn test_less_than_or_equal_empty() {
        let op = LessThanOrEqualOperation::new();
        let ctx = create_test_context();

        // Test with empty operand
        let args = vec![FhirPathValue::Integer(3), FhirPathValue::Empty];
        let result = op.evaluate(&args, &ctx).await.unwrap();
