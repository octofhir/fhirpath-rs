...
mod tests {
    ...
    fn create_test_context(input: FhirPathValue) -> EvaluationContext {
        EvaluationContext {
            input,
            model_provider: Arc::new(octofhir_fhirpath_model::MockModelProvider::new()),
    ...
    fn test_count_function() {
        ...
        let empty_collection = FhirPathValue::Collection(
    ...
    fn test_empty_function() {
        ...
        let collection = FhirPathValue::Collection(
            octofhir_fhirpath_model::Collection::from(vec![FhirPathValue::Integer(1)])
        );
        let context = create_test_context(collection);
        let result = func.execute(&[], &context).unwrap();
