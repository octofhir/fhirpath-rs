...
mod tests {
    ...
    #[test]
    fn test_has_template_id_of() {
        let op = HasTemplateIdOfFunction::new();

        // Test CDA document with template ID
        let cda_doc = json!({
        ...
        // Test FHIR resource with profile
        ...
        // Test resource without matching template ID
        let context = create_context(FhirPathValue::JsonValue(fhir_resource.clone().into()));
        let args = vec![FhirPathValue::String("http://different.profile".into())];
        let result = op.execute(&args, &context).unwrap();
