...
impl EndsWithFunction {
    ...
    fn create_metadata() -> OperationMetadata {
        MetadataBuilder::new("endsWith", OperationType::Function)
            ...
            .parameter("suffix", TypeConstraint::Specific(FhirPathType::String), false)
    ...
    fn evaluate_ends_with(
        ...
        match input {
            FhirPathValue::Collection(items) => {
                ...
                return self.process_single_value(value, suffix);
            }
            _ => {
                // Process as single value
                return self.process_single_value(input, suffix);
            }
        }
    }
...
mod tests {
    ...
    fn test_sync_evaluation() {
        ...
        let sync_result = ends_with_fn.try_evaluate_sync(&args, &context).unwrap().unwrap();
        assert_eq!(sync_result, FhirPathValue::Boolean(true));
        assert!(ends_with_fn.supports_sync());
    }
    ...
    fn test_error_conditions() {
        ...
        let args = vec![FhirPathValue::String("test".into())];
        let result = ends_with_fn.try_evaluate_sync(&args, &context).unwrap();
