...
impl IsOperation {
    ...
    async fn matches_type(&self, value: &FhirPathValue, type_name: &str, context: &EvaluationContext) -> bool {
        ...
        if context.model_provider.is_value_of_type(value, type_name).await {
            return true;
        }
        ...
        if !type_name.starts_with("FHIR.") {
            ...
            if context.model_provider.is_value_of_type(value, &fhir_type_name).await {
                return true;
            }
        }
        ...
        if type_name.starts_with("FHIR.") {
            let bare_type_name = &type_name[5..];
            if context.model_provider.is_value_of_type(value, bare_type_name).await {
                return true;
            }
        }
...
impl AsyncOperation for IsOperation {
    ...
    async fn execute(&self, args: &[FhirPathValue], context: &EvaluationContext) -> Result<FhirPathValue> {
        ...
        let type_name = match &args[0] {
            ...
            FhirPathValue::TypeInfoObject { namespace, name } => {
                ...
                if namespace.as_ref() == "FHIR" {
                    ...
                    return Ok(FhirPathValue::Boolean(
                        self.matches_type(&context.input, &format!("{}.{}", namespace, name), context).await
                    ));
                }
            }
                ...
                if col.len() == 1 {
                    if let Some(FhirPathValue::TypeInfoObject { namespace, name }) = col.get(0) {
                        ...
                        return Err(FhirPathError::TypeError {
                            message: format!("is() type argument must be a type identifier or string, got {}", 
            ...
            other => {
                return Err(FhirPathError::TypeError {
                    message: format!("is() type argument must be a type identifier or string, got {}", other.type_name()),
                });
            }
        };
