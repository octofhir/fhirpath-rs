...
impl AsyncOperation for AsOperation {
    ...
    async fn execute(&self, args: &[FhirPathValue], context: &EvaluationContext) -> Result<FhirPathValue> {
        ...
        let type_name = match &args[0] {
                    ...
                    return match &context.input {
                        FhirPathValue::Collection(col) => {
                            ...
                            for item in col.iter() {
                                if let Some(cast_item) = self.try_cast_to_type(item, &full_type_name) {
                ...
                if col.len() == 1 {
                    if let Some(FhirPathValue::TypeInfoObject { namespace, name }) = col.get(0) {
                        ...
                        return Err(FhirPathError::TypeError {
                            message: format!("as() type argument must be a type identifier or string, got {}", 
            ...
            other => {
                return Err(FhirPathError::TypeError {
                    message: format!("as() type argument must be a type identifier or string, got {}", other.type_name()),
                });
            }
        };
