...
mod tests {
    ...
    #[tokio::test]
    async fn test_first_empty_collection() {
    ...
    #[tokio::test]
    async fn test_first_single_item() {
        let first_fn = FirstFunction::new();
    ...
    #[tokio::test]
    async fn test_first_multiple_items() {
        let first_fn = FirstFunction::new();
        ...
        assert_eq!(
            result,
            FhirPathValue::collection(vec![FhirPathValue::String("first".into())])
        );
    }

    #[tokio::test]
    async fn test_first_with_arguments_error() {
        let first_fn = FirstFunction::new();
    ...
    fn test_sync_evaluation() {
        ...
        let sync_result = first_fn.try_evaluate_sync(&[], &context).unwrap().unwrap();
        assert_eq!(
            sync_result,
            FhirPathValue::collection(vec![FhirPathValue::String("first".into())])
        );
        assert!(first_fn.supports_sync());
    }

    #[test]
    fn test_metadata() {
        let first_fn = FirstFunction::new();
        let metadata = first_fn.metadata();

        assert_eq!(metadata.basic.name, "first");
