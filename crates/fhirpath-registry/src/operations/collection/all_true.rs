...
impl AllTrueFunction {
    ...
    fn create_metadata() -> OperationMetadata {
        MetadataBuilder::new("allTrue", OperationType::Function)
            .description("Returns true if all boolean items in the collection are true. Returns true for an empty collection. Non-boolean items are ignored.")
...
impl FhirPathOperation for AllTrueFunction {
    ...
    fn metadata(&self) -> &OperationMetadata {
        static METADATA: std::sync::LazyLock<OperationMetadata> = std::sync::LazyLock::new(|| {
            AllTrueFunction::create_metadata()
        });
        &METADATA
    }
...
impl AllTrueFunction {
    fn evaluate_all_true(&self, args: &[FhirPathValue], context: &EvaluationContext) -> Result<FhirPathValue> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_all_true_empty_collection() {
        let all_true_fn = AllTrueFunction::new();
        let empty_collection = FhirPathValue::collection(vec![]);
        let context = create_test_context(empty_collection);
        
        let result = all_true_fn.evaluate(&[], &context).await.unwrap();
        assert_eq!(result, FhirPathValue::Boolean(true));
    }

    #[tokio::test]
    async fn test_all_true_single_boolean() {
        let all_true_fn = AllTrueFunction::new();
    ...
    #[tokio::test]
    async fn test_all_true_multiple_booleans() {
        let all_true_fn = AllTrueFunction::new();
    ...
    #[tokio::test]
    async fn test_all_true_mixed_types() {
        let all_true_fn = AllTrueFunction::new();
    ...
    #[tokio::test]
    async fn test_all_true_no_booleans() {
        let all_true_fn = AllTrueFunction::new();
    ...
    #[tokio::test]
    async fn test_all_true_with_arguments_error() {
        let all_true_fn = AllTrueFunction::new();
        let collection = FhirPathValue::collection(vec![FhirPathValue::Boolean(true)]);
        let context = create_test_context(collection);
        
        let result = all_true_fn.evaluate(&[FhirPathValue::Boolean(true)], &context).await;
    ...
    fn test_sync_evaluation() {
        ...
        let sync_result = all_true_fn.try_evaluate_sync(&[], &context).unwrap().unwrap();
        assert_eq!(sync_result, FhirPathValue::Boolean(true));
        assert!(all_true_fn.supports_sync());
    }

    #[test]
    fn test_metadata() {
        let all_true_fn = AllTrueFunction::new();
        let metadata = all_true_fn.metadata();

        assert_eq!(metadata.basic.name, "allTrue");
