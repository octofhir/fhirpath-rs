...
impl EvaluationContext {
    ...
    /// Only includes values that affect function evaluation results
    pub fn compute_cache_hash(&self) -> u64 {
        use std::hash::{Hash, Hasher};
        let mut hasher = rustc_hash::FxHasher::default();

        // Hash current input value (simplified - real implementation might be more sophisticated)
        format!("{:?}", self.input).hash(&mut hasher);

        // Hash variables that might affect function results
...
impl FunctionRegistry {
    ...
    pub fn evaluate_function(
        ...
        if self.is_pure_function(name) && self.cache_config.enable_result_caching {
            let result_key =
                crate::cache::generate_result_cache_key(name, args, context.compute_cache_hash());
