...
//! Unified convertsToInteger() function implementation for FHIRPath
...
/// Unified convertsToInteger() function implementation
/// 
/// Checks if a value can be converted to an integer
pub struct UnifiedConvertsToIntegerFunction {
    metadata: EnhancedFunctionMetadata,
}

impl UnifiedConvertsToIntegerFunction {
    pub fn new() -> Self {
        let metadata = MetadataBuilder::new("convertsToInteger", crate::function::FunctionCategory::TypeConversion)
            .display_name("Converts To Integer")
            .description("Returns true if the value can be converted to an integer")
            .example("'42'.convertsToInteger()")
            .example("'not-a-number'.convertsToInteger()")
            .output_type(TypePattern::Exact(TypeInfo::Boolean))
            .execution_mode(ExecutionMode::Sync)
            .pure(true)
            .lsp_snippet("convertsToInteger()")
            .keywords(vec!["convertsToInteger", "integer", "convert", "check", "validation"])
            .usage_pattern(
...
#[async_trait]
impl UnifiedFhirPathFunction for UnifiedConvertsToIntegerFunction {
    fn name(&self) -> &str {
        "convertsToInteger"
    }
    ...
    fn evaluate_sync(
        ...
        let can_convert = match &context.input {
            ...
            FhirPathValue::Decimal(d) => d.to_i64().is_some(),
            FhirPathValue::String(s) => s.trim().parse::<i64>().is_ok(),
...
mod tests {
    ...
    #[tokio::test]
    async fn test_unified_convertsToInteger_function() {
        let converts_func = UnifiedConvertsToIntegerFunction::new();
        ...
        // Test metadata
        assert_eq!(converts_func.name(), "convertsToInteger");
