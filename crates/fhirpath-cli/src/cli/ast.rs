...
pub fn extract_resource_type(resource: &JsonValue) -> Option<String> {
    resource.get("resourceType").and_then(|rt| rt.as_str()).map(|s| s.to_string())
}
                ...
                BinaryOperator::Union => {
                    ...
                    if let Some(ref args) = ast_node.arguments {
                        if let Some(left_arg) = args.get(0) {
                            left_arg.return_type.clone().map(|t| format!("{}[]", t.trim_end_matches("[]")))
                        } else {
        ...
        ExpressionNode::IndexAccess(node) => {
            ...
            if let Some(ref mut args) = ast_node.arguments {
                if let Some(object_arg) = args.get_mut(0) {
                    ...
                    let return_type = enhanced_object.return_type.as_ref()
        ...
        ExpressionNode::Collection(node) => {
            ...
            if let Some(ref args) = ast_node.arguments {
                if let Some(first_item) = args.get(0) {
                    ast_node.return_type = first_item.return_type.clone()
...
pub fn convert_ast_to_lab_format(
    ...
    match ast {
        ...
        ExpressionNode::PropertyAccess(node) => {
            let object_arg = convert_ast_to_lab_format(&node.object, function_registry, model_provider);
        ...
        ExpressionNode::FunctionCall(node) => {
            ...
            for arg in &node.arguments {
                args.push(convert_ast_to_lab_format(arg, function_registry, model_provider));
            }
        ...
        ExpressionNode::BinaryOperation(node) => {
            ...
            let right_arg = convert_ast_to_lab_format(&node.right, function_registry, model_provider);
        ...
        ExpressionNode::IndexAccess(node) => {
            let object_arg = convert_ast_to_lab_format(&node.object, function_registry, model_provider);
            let index_arg = convert_ast_to_lab_format(&node.index, function_registry, model_provider);
        ...
        ExpressionNode::UnaryOperation(node) => {
            let operand_arg = convert_ast_to_lab_format(&node.operand, function_registry, model_provider);
        ...
        ExpressionNode::Collection(node) => {
            ...
            for item in &node.elements {
                args.push(convert_ast_to_lab_format(item, function_registry, model_provider));
            }
