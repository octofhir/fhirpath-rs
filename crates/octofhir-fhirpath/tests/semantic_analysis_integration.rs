...
async fn test_valid_expression_analysis() {
    ...
    assert!(result.ast.is_some(), "Valid expression should parse successfully");
    assert!(result.analysis.success, "Valid expression should pass semantic analysis");
    assert!(result.analysis.diagnostics.is_empty(), "Valid expression should have no diagnostics");
}
...
async fn test_invalid_property_semantic_error() {
    ...
    assert!(result.ast.is_some(), "Expression should parse syntactically");
    assert!(!result.analysis.success, "Semantic analysis should fail for invalid property");

    // Should have error diagnostic
    let errors: Vec<_> = result.analysis.diagnostics.iter()
    ...
    assert!(error_message.contains("given1"), "Error should mention the invalid property");
    assert!(error_message.contains("not found"), "Error should indicate property not found");
...
async fn test_unknown_resource_type_error() {
    ...
    let has_resource_error = result.analysis.diagnostics.iter()
...
async fn test_semantic_vs_runtime_behavior() {
    ...
    assert!(result.ast.is_some(), "Should parse for potential runtime evaluation");
    assert!(!result.analysis.success, "Semantic analysis should catch the error");

    let error_count = result.analysis.diagnostics.iter()
        .filter(|d| matches!(d.severity, DiagnosticSeverity::Error))
        .count();
