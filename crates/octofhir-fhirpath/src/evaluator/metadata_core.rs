...
use crate::{
    ast::{ExpressionNode, IdentifierNode, LiteralNode, VariableNode, LiteralValue},
    core::{Collection, FhirPathError, FhirPathValue, Result},
    evaluator::{
        traits::MetadataAwareEvaluator,
        EvaluationContext,
    },
    path::CanonicalPath,
...
impl MetadataCoreEvaluator {
    ...
    async fn evaluate_identifier(
        ...
        if let Some(first_char) = name.chars().next() {
            if first_char.is_uppercase() {
                return self.evaluate_resource_type_filter(name, context, resolver).await;
            }
        }
        ...
        for wrapped_value in start_context {
            let property_results = self.navigate_property_on_value(&wrapped_value, name, resolver).await?;
        ...
        match source.as_plain() {
            ...
            FhirPathValue::Collection(values) => {
                ...
                for (i, value) in values.iter().enumerate() {
                    ...
                    let property_results = Box::pin(self
                        .navigate_property_on_value(&wrapped_element, property, resolver))
                        .await?;
    ...
    async fn evaluate_resource_type_filter(
        ...
        if !resolver.model_provider().resource_type_exists(resource_type).unwrap_or(false) {
    ...
    async fn collection_to_wrapped(
        ...
        for (i, value) in collection.iter().enumerate() {
            let wrapped = self.value_to_wrapped_with_inference(value, i, resolver).await?;
    ...
    async fn evaluate_with_metadata(
        ...
        match expr {
            ExpressionNode::Identifier(identifier) => {
                self.evaluate_identifier(identifier, context, resolver).await
            }
            ExpressionNode::Literal(literal) => {
                self.evaluate_literal(literal, context, resolver)
            }
...
mod tests {
    use super::*;
    use crate::ast::{IdentifierNode, LiteralNode, VariableNode, LiteralValue};
