...
impl ValueNavigator for Navigator {
    fn navigate_property(
        ...
        match value {
            FhirPathValue::Resource(json) => {
                self.navigate_json_property(json, property)
            },
    ...
    fn navigate_index(&self, value: &FhirPathValue, index: usize) -> Result<FhirPathValue> {
        match value {
            FhirPathValue::Resource(json) => {
                self.navigate_json_index(json, index)
            },
            FhirPathValue::JsonValue(json) => {
                self.navigate_json_index(json, index)
            },
...
mod tests {
    ...
    impl ModelProvider for MockModelProvider {
        ...
        async fn is_choice_type_property(&self, _resource_type: &str, _property: &str) -> Result<bool> {
            Ok(false)
        }
        async fn resolve_reference(&self, _reference: &str, _current_resource: Option<&FhirPathValue>) -> Result<Option<FhirPathValue>> {
            Ok(None)
        }
        async fn get_property_type(&self, _resource_type: &str, _property: &str) -> Result<Option<String>> {
            Ok(None)
        }
    }
    use serde_json::json;
    ...
    fn test_simple_property_navigation() {
        ...
        let id_result = navigator.navigate_property(&patient, "id", &provider).unwrap();
        assert_eq!(id_result, FhirPathValue::String("example".to_string()));

        let active_result = navigator.navigate_property(&patient, "active", &provider).unwrap();
    ...
    fn test_array_property_navigation() {
        ...
        let telecom_result = navigator.navigate_property(&patient, "telecom", &provider).unwrap();
    ...
    fn test_choice_type_navigation() {
        ...
        let multiple_birth_result = navigator.navigate_property(&patient, "multipleBirth", &provider).unwrap();
        assert_eq!(multiple_birth_result, FhirPathValue::Integer(1));

        // Test direct access to choice type property
        let multiple_birth_int_result = navigator.navigate_property(&patient, "multipleBirthInteger", &provider).unwrap();
    ...
    fn test_missing_property_navigation() {
        ...
        let missing_result = navigator.navigate_property(&patient, "nonexistent", &provider).unwrap();
    ...
    fn test_path_navigation() {
        ...
        assert_eq!(simple_path_result, FhirPathValue::String("example".to_string()));
    ...
    fn test_collection_property_navigation() {
        ...
        let ids_result = navigator.navigate_property(&collection, "id", &provider).unwrap();
        match ids_result {
        ...
        let active_result = navigator.navigate_property(&collection, "active", &provider).unwrap();
        match active_result {
    ...
    fn test_primitive_value_navigation() {
        ...
        let string_result = navigator.navigate_property(&string_value, "String", &provider).unwrap();
        assert_eq!(string_result, string_value);

        // Non-matching property should return empty
        let empty_result = navigator.navigate_property(&string_value, "Integer", &provider).unwrap();
        assert_eq!(empty_result, FhirPathValue::Empty);

        let empty_result2 = navigator.navigate_property(&string_value, "someProperty", &provider).unwrap();
