...
impl ExpressionEvaluator {
    ...
    fn evaluate_expression<'a>(
        ...
        let result = match node {
            ...
            ExpressionNode::Collection(collection_node) => {
                self.evaluate_collection_literal(collection_node, context).await
            },
            ...
            ExpressionNode::Union(union_node) => {
                self.evaluate_union(union_node, context).await
            },
            ...
            _ => {
                ...
                self.warnings.push(EvaluationWarning {
                    code: "W002".to_string(),
                    message: format!("Expression type '{}' not yet fully implemented", node.node_type()),
                    location: node.location().map(|loc| loc.offset..(loc.offset + loc.length)),
                });
                Ok(Collection::empty())
            }
        ...
        for item in current_context.iter() {
            let resource_type_opt = match item {
                FhirPathValue::Resource(map) | FhirPathValue::JsonValue(map) => {
                    map.get("resourceType").and_then(|v| v.as_str()).map(|s| s.to_string())
                }
                _ => None,
            };
        ...
        match binary_node.operator {
            ...
            BinaryOperator::NotEqual => {
                ...
                if left_result.len() == 1 && right_result.len() == 1 {
                    ...
                    let incomparable_not_equal = matches!((l, r),
                        ...
                        matches!(l, FhirPathValue::String(s) if crate::core::temporal::PrecisionDate::parse(s).is_some()) && matches!(r, FhirPathValue::Time(_))
                    ) || (
                        matches!(r, FhirPathValue::String(s) if crate::core::temporal::PrecisionDate::parse(s).is_some()) && matches!(l, FhirPathValue::Time(_))
                    );
            ...
            BinaryOperator::LessThan => {
                self.apply_binary_ordering_op(left_result, right_result, |ord| ord == std::cmp::Ordering::Less)
            },
            BinaryOperator::GreaterThan => {
                self.apply_binary_ordering_op(left_result, right_result, |ord| ord == std::cmp::Ordering::Greater)
            },
            ...
            BinaryOperator::In => {
                self.apply_in_operator(left_result, right_result)
            },
        ...
        F: Fn(std::cmp::Ordering) -> bool,
    {
        ...
        let ord_opt: Option<std::cmp::Ordering> = match (left, right) {
            ...
            (FhirPathValue::DateTime(a), FhirPathValue::DateTime(b)) => Some(a.datetime.cmp(&b.datetime)),
    ...
    fn fhirpath_values_equivalent(&self, left: &FhirPathValue, right: &FhirPathValue) -> Option<bool> {
        use rust_decimal::Decimal;
        use std::str::FromStr;
    ...
    async fn evaluate_unary_operation(
        ...
        let operand_result = self.evaluate_expression(&unary_node.operand, context).await?;
    ...
    async fn evaluate_method_call(
        ...
        for arg in &method_node.arguments {
            ...
            let needs_type_shorthand = matches!(method_node.method.as_str(), "is" | "as" | "ofType");
            if needs_type_shorthand {
                if let Some(qname) = extract_qualified_name(arg) {
                    argument_results.push(crate::core::Collection::single(crate::core::FhirPathValue::String(qname)));
                    continue;
                }
            }
        ...
        for arg_collection in &argument_results {
            let arg_values: Vec<crate::core::FhirPathValue> = arg_collection.iter().cloned().collect();
            argument_value_vecs.push(arg_values);
        }

        let function_context = FunctionContext {
            input: &input_values,
            arguments: if argument_value_vecs.is_empty() { &[] } else { &argument_value_vecs[0] },
        ...
        if let Some((function, _metadata)) = self.function_registry.get_sync_function(&method_node.method) {
        ...
        if let Some((async_function, _metadata)) = self.function_registry.get_async_function(&method_node.method) {
    ...
    fn is_lambda_function(&self, method_name: &str) -> bool {
        matches!(method_name, "select" | "where" | "all" | "repeat" | "aggregate" | "sort")
    }
    ...
    async fn evaluate_lambda_method_call(
        ...
        match method_node.method.as_str() {
            "select" => {
                self.memory_allocations += 1;
                lambda_evaluator.evaluate_select(&object_result, lambda_expr, &adapter).await
            },
            "where" => {
                self.memory_allocations += 1;
                lambda_evaluator.evaluate_where(&object_result, lambda_expr, &adapter).await
            },
            "all" => {
                self.memory_allocations += 1;
                let result = lambda_evaluator.evaluate_all(&object_result, lambda_expr, &adapter).await?;
                Ok(Collection::single(crate::core::FhirPathValue::Boolean(result)))
            },
            "repeat" => {
                self.memory_allocations += 1;
                lambda_evaluator.evaluate_repeat(&object_result, lambda_expr, &adapter, None, None).await
            },
            "aggregate" => {
                ...
                let initial_value = if method_node.arguments.len() >= 2 {
                    let init_result = self.evaluate_expression(&method_node.arguments[1], context).await?;
    ...
    async fn evaluate_property_access(
        ...
        let object_result = self.evaluate_expression(&property_node.object, context).await?;
    ...
    async fn evaluate_index_access(
        ...
        let object_result = self.evaluate_expression(&index_node.object, context).await?;
    ...
    async fn evaluate_type_cast(
        ...
        let expression_result = self.evaluate_expression(&typecast_node.expression, context).await?;
        let mut result = Collection::empty();
    ...
    async fn evaluate_type_check(
        ...
        let expression_result = self.evaluate_expression(&typecheck_node.expression, context).await?;
        let mut result = Collection::empty();
    ...
    async fn evaluate_function_call(
                ...
                match function_node.name.as_str() {
                    "select" => {
                        ...
                        return lambda_evaluator
                            .evaluate_select(input_collection, &function_node.arguments[0], &adapter)
                            .await;
                    }
                    "where" => {
                        if function_node.arguments.len() != 1 { 
                    ...
                    "repeat" => {
                        ...
                        self.memory_allocations += 1;
                        return lambda_evaluator
                            .evaluate_repeat(input_collection, &function_node.arguments[0], &adapter, None, None)
                            .await;
                    }
                    "aggregate" => {
                        ...
                        let initial_value = if function_node.arguments.len() >= 2 {
                            let init_res = self.evaluate_expression(&function_node.arguments[1], context).await?;
                            if init_res.is_empty() { None } else { Some(init_res.first().unwrap().clone()) }
                        } else { None };
                        self.memory_allocations += 1;
                        let val = lambda_evaluator
                            .evaluate_aggregate(input_collection, lambda_expr, initial_value, &adapter)
                            .await?;
                    ...
                    "sort" => {
                        ...
                        for arg in &function_node.arguments {
                            let (descending, expr) = match arg {
                                crate::ast::ExpressionNode::UnaryOperation(un) if matches!(un.operator, crate::ast::UnaryOperator::Negate) => (true, (*un.operand).clone()),
                                _ => (false, arg.clone()),
                            };
                            criteria.push(crate::evaluator::lambda::SortCriterion { expression: expr, descending });
                        }
                        self.memory_allocations += 1;
                        return lambda_evaluator
        ...
        // Try sync function first
        if let Some((function, _metadata)) = self.function_registry.get_sync_function(&function_node.name) {
            match function(&function_context) {
                Ok(result) => {
                    self.function_calls += 1;
                    self.memory_allocations += 1;
        ...
        // Try async function if sync not found
        if let Some((async_function, _metadata)) = self.function_registry.get_async_function(&function_node.name) {
            match async_function(&function_context).await {
                Ok(result) => {
                    self.function_calls += 1;
                    self.memory_allocations += 1;
            ...
            FhirPathValue::Resource(resource) => {
                if let Some(value) = resource.get(property) {
                    ...
                    if let Some(arr) = value.as_array() {
                        let items: Vec<FhirPathValue> = arr.iter().map(|v| json_to_fhir_value(v)).collect();
                    ...
                    if let Some(obj) = resource.as_object() {
                        if let Some((_, choice_val)) = obj.iter().find(|(k, _)| k.starts_with(property) && k.chars().nth(property.len()).map(|c| c.is_uppercase()).unwrap_or(false)) {
    ...
    fn cast_value(&self, value: &FhirPathValue, target_type: &str) -> Result<Option<FhirPathValue>> {
        match target_type.to_lowercase().as_str() {
            ...
            "decimal" => {
                match value {
                    ...
                    FhirPathValue::Integer(i) => {
                        Ok(Some(FhirPathValue::decimal(rust_decimal::Decimal::from(*i))))
                    },
                    FhirPathValue::String(s) => {
...
impl ExpressionEvaluatorAdapter {
    ...
    async fn eval_async(&self, expr: &crate::ast::ExpressionNode, context: &crate::evaluator::EvaluationContext) -> crate::core::Result<crate::core::Collection> {
