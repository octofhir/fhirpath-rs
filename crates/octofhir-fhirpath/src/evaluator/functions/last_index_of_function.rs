...
//!
//! The lastIndexOf function finds the index of the last occurrence of a substring in a string
//! or element in a collection. Returns -1 if not found.
//! Syntax: string.lastIndexOf(substring) or collection.lastIndexOf(item)
...
impl LastIndexOfFunctionEvaluator {
    ...
    pub fn create() -> Arc<dyn PureFunctionEvaluator> {
        Arc::new(Self {
            metadata: FunctionMetadata {
                ...
                description: "Returns the index of the last occurrence of a substring in a string or an item in a collection. Returns -1 if not found.".to_string(),
    ...
    /// Find last substring index in string
    fn find_last_substring_index(haystack: &str, needle: &str) -> i64 {
    ...
    /// Find last item index in collection
    fn find_last_item_index(collection: &[FhirPathValue], search_item: &FhirPathValue) -> i64 {
    ...
    async fn evaluate(
        ...
        if args.len() != 1 {
            return Err(FhirPathError::evaluation_error(
                ...
                "lastIndexOf function requires exactly one argument (searchValue)".to_string(),
            ));
        }
            ...
            match &input[0] {
                FhirPathValue::String(haystack, _, _) => {
                    ...
                    if let FhirPathValue::String(needle, _, _) = search_value {
                        Self::find_last_substring_index(haystack, needle)
                    } else {
