...
//!
//! Computes the highest possible value for the supplied input at the requested precision.
...
impl HighBoundaryFunctionEvaluator {
    pub fn create() -> Arc<dyn PureFunctionEvaluator> {
        Arc::new(Self {
            metadata: FunctionMetadata {
                name: "highBoundary".to_string(),
                description: "Returns the greatest possible value that could be represented by the input at the supplied precision.".to_string(),
...
impl PureFunctionEvaluator for HighBoundaryFunctionEvaluator {
    async fn evaluate(
        &self,
        ...
        if args.len() > 1 {
            return Err(FhirPathError::evaluation_error(
                FP0053,
                "highBoundary accepts at most one precision argument".to_string(),
            ));
        }
        ...
        if input.len() != 1 {
            return Err(FhirPathError::evaluation_error(
                FP0054,
                "highBoundary expects a singleton input".to_string(),
            ));
        }
        ...
        let result = match &input[0] {
            ...
            FhirPathValue::Date(date, type_info, primitive) => {
                match resolve_date_precision(date.precision, precision_value) {
                    Ok(target_precision) => {
                        let adjusted =
                            compute_date_boundary(date, target_precision, BoundaryKind::High);
            ...
            FhirPathValue::Time(time, type_info, primitive) => {
                match resolve_time_precision(time.precision, precision_value) {
                    Ok(target_precision) => {
                        let adjusted =
                            compute_time_boundary(time, target_precision, BoundaryKind::High);
            ...
            other => {
                return Err(FhirPathError::evaluation_error(
                    FP0055,
                    format!("highBoundary cannot be applied to {}", other.type_name()),
                ));
            }
        };
