...
impl ValueMetadata {
    ...
    pub fn infer_from_value(
        value: &crate::core::FhirPathValue,
        path: CanonicalPath,
    ) -> Self {
...
pub mod collection_utils {
    ...
    pub fn to_plain_collection(wrapped: WrappedCollection) -> Collection {
        let values: Vec<FhirPathValue> = wrapped.into_iter()
            .map(|w| w.value)
            .collect();
...
mod tests {
    ...
    fn test_collection_utilities() {
        ...
        let wrapped_collection = collection_utils::from_plain_with_base_metadata(values, base_metadata);
