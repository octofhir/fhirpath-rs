...
mod tests {
    ...
    use super::super::terminology_utils::{
        Coding, ConceptDesignation, ConceptTranslation, TerminologyUtils,
    };
    ...
    fn test_coding_extraction_from_fhir_values() {
        ...
        let codings =
            TerminologyUtils::extract_all_codings(&FhirPathValue::Resource(codeable_concept_json))
                .unwrap();
    ...
    fn test_concept_translation_utilities() {
        ...
        if let FhirPathValue::Resource(json_val) = fhir_value {
            ...
            assert_eq!(
                obj.get("equivalence").unwrap().as_str().unwrap(),
                "equivalent"
            );
            assert!(obj.contains_key("concept"));
            assert_eq!(
                obj.get("comment").unwrap().as_str().unwrap(),
                "Standard gender mapping"
            );

            let concept = obj.get("concept").unwrap().as_object().unwrap();
            assert_eq!(
                concept.get("system").unwrap().as_str().unwrap(),
    ...
    fn test_concept_designation_utilities() {
        ...
        if let FhirPathValue::Resource(json_val) = fhir_value {
            ...
            assert_eq!(
                use_obj.get("system").unwrap().as_str().unwrap(),
                "http://terminology.hl7.org/CodeSystem/designation-usage"
            );
    ...
    async fn test_mock_terminology_provider() {
        ...
        let is_member = provider
            .check_valueset_membership(
                &coding,
        ...
        let translations = provider
            .translate_concept(
                &source_coding,
                "http://example.org/ConceptMap/gender-mapping",
                false,
            )
            .await
            .unwrap();
        ...
        let is_valid = provider
            .validate_code("http://hl7.org/fhir/administrative-gender", "male")
            .await
            .unwrap();
        assert!(is_valid);

        let is_invalid = provider
            .validate_code("http://hl7.org/fhir/administrative-gender", "unknown")
            .await
            .unwrap();
        assert!(!is_invalid);
        ...
        let subsumes = provider
            .check_subsumption(&parent_coding, &child_coding)
            .await
            .unwrap();
        assert!(subsumes);

        let unrelated_coding = Coding::new("http://example.org/codes", "other");
        let does_not_subsume = provider
            .check_subsumption(&parent_coding, &unrelated_coding)
            .await
            .unwrap();
        ...
        let designations_en = provider
            .get_designations(&male_coding, Some("en"), None)
            .await
            .unwrap();
        ...
        let designations_es = provider
            .get_designations(&male_coding, Some("es"), None)
            .await
            .unwrap();
        ...
        let properties = provider
            .get_concept_properties(&male_coding, "definition")
            .await
            .unwrap();
        ...
        let empty_designations = provider
            .get_designations(&unknown_coding, None, None)
            .await
            .unwrap();
        assert!(empty_designations.is_empty());

        let empty_properties = provider
            .get_concept_properties(&unknown_coding, "definition")
            .await
            .unwrap();
        assert!(empty_properties.is_empty());

        let empty_expansions = provider
            .expand_valueset("http://example.org/ValueSet/empty")
            .await
            .unwrap();
    ...
    fn test_default_terminology_provider_creation() {
        ...
        let provider_with_custom_url =
            DefaultTerminologyProvider::with_server_url("https://custom-tx.example.com/r4");
    ...
    fn test_edge_cases() {
        ...
        assert!(TerminologyUtils::codings_equal(
            &original_coding,
            &extracted_coding
        ));
    }
    ...
    fn test_complex_scenarios() {
        ...
        let first_coding = TerminologyUtils::extract_coding(&FhirPathValue::Resource(json!({
            "coding": [
                {
                    "system": "http://loinc.org",
                    "code": "789-8",
    ...
    async fn test_async_error_propagation() {
        ...
        let result = provider
            .check_subsumption(&coding_a, &coding_b)
            .await
            .unwrap();
