...
/// }
/// ```
pub fn parse_ast_with_mode(input: &str, mode: ParsingMode) -> Result<ExpressionNode, FhirPathError> {
...
fn parse_analysis(input: &str) -> ParseResult {
    ...
    match result.ast {
        ...
        None => {
            let error_message = result.diagnostics.first()
                .map(|d| d.message.clone())
...
fn split_on_semicolons(input: &str) -> Vec<String> {
    ...
    if expressions.is_empty() && !input.trim().is_empty() && !input.trim().chars().all(|c| c == ';' || c.is_whitespace()) {
...
mod tests {
    ...
    fn test_parser_use_case_recommendations() {
        ...
        assert_eq!(
            recommend_mode(ParserUseCase::IDE),
    ...
    fn test_performance_characteristics() {
        ...
        assert!(fast_ops_per_sec > 5_000.0, 
            "Fast parser too slow: {} ops/sec", fast_ops_per_sec);
        assert!(analysis_ops_per_sec > 1_000.0, 
            "Analysis parser too slow: {} ops/sec", analysis_ops_per_sec);
    ...
    fn test_complex_expressions() {
        ...
        assert!(fast_result.success, "Fast parser should handle complex expressions");
        
        let analysis_result = parse_with_mode(complex_expr, ParsingMode::Analysis);
        assert!(analysis_result.success, "Analysis parser should handle complex expressions");
    }
    ...
    fn test_various_error_conditions() {
        ...
        for (expr, description) in error_cases {
            ...
            assert!(!result.diagnostics.is_empty(), "Should have diagnostics for: {}", description);
        }
    }
