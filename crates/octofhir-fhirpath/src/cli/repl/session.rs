...
use super::{ReplConfig, ReplCommand};
...
impl ReplSession {
    ...
    pub fn with_engine(engine: FhirPathEngine, config: ReplConfig) -> Result<Self> {
        ...
        let analyzer = FhirPathAnalyzer::with_config(engine.model_provider().clone(), analyzer_config);
    ...
    async fn execute_command(&mut self, command: ReplCommand) -> Result<Option<String>> {
        match command {
            ReplCommand::Load { path } => {
                match self.load_resource_from_file(&path) {
                    Ok(()) => {
            ...
            ReplCommand::Set { name, value } => {
                match self.set_variable(&name, &value) {
                    Ok(()) => {
                        let success_msg = if self.config.color_output {
                            format!("✅ Variable '{name}' set to '{value}'")
                        } else {
                            format!("Variable '{name}' set to '{value}'")
                        };
                        Ok(Some(success_msg))
                    ...
                    Err(e) => {
                        let error_msg = if self.config.color_output {
                            format!("❌ Failed to set variable '{name}': {e}\n💡 Variables should be simple string values")
                        } else {
                            format!("Failed to set variable '{name}': {e}\nTip: Variables should be simple string values")
                        };
                        Err(anyhow::anyhow!(error_msg))
                    }
                }
            ...
            ReplCommand::Unset { name } => {
                if self.variables.contains_key(&name) {
                    ...
                    let warning_msg = if self.config.color_output {
                        format!("⚠️ Variable '{name}' not found. Use ':vars' to see defined variables.")
                    } else {
                        format!("Warning: Variable '{name}' not found. Use ':vars' to see defined variables.")
                    };
                    Ok(Some(warning_msg))
                }
            }
            ReplCommand::Vars => {
                Ok(Some(self.list_variables()))
            }
                ...
                match &self.current_resource {
                    ...
                    None => {
                        let msg = if self.config.color_output {
                            "ℹ️ No resource loaded. Use ':load <file>' to load a FHIR resource.".to_string()
                        } else {
                            "No resource loaded. Use ':load <file>' to load a FHIR resource.".to_string()
                        };
                        Ok(Some(msg))
                    }
                }
    ...
    async fn evaluate_expression(&self, expression: &str) -> Result<String> {
        ...
        if expression.trim().is_empty() {
            return Ok("Empty expression. Try typing a FHIRPath expression or ':help' for assistance.".to_string());
        }
        ...
        let result = if self.variables.is_empty() {
            self.engine.evaluate(expression, input_json).await
                .with_context(|| format!("Failed to evaluate expression: '{}'", expression))?
        } else {
            // Convert our variables to the format expected by the engine
            let variables: std::collections::HashMap<String, FhirPathValue> = 
                self.variables.iter()
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();
                    
            self.engine.evaluate_with_variables(expression, input_json, variables).await
                .with_context(|| format!("Failed to evaluate expression with variables: '{}'", expression))?
        };
        
        let duration = start.elapsed();
        let mut output = self.formatter.format_result(&result, self.config.show_types);
        
        // Add performance information for longer evaluations
        if duration.as_millis() > 100 {
            let timing_info = if self.config.color_output {
                format!("\n⏱️ Evaluation took {:.2}ms", duration.as_secs_f64() * 1000.0)
            } else {
    ...
    pub fn set_variable(&mut self, name: &str, value: &str) -> Result<()> {
        let parsed_value = self.parse_variable_value(value)?;
        self.variables.insert(name.to_string(), parsed_value);
        Ok(())
    }

    /// Parse a string value into a FHIRPath value
    fn parse_variable_value(&self, value: &str) -> Result<FhirPathValue> {
        // Try to parse as JSON first
        if let Ok(json_val) = serde_json::from_str::<JsonValue>(value) {
            Ok(FhirPathValue::resource_from_json(json_val))
        } else {
            // Treat as string literal
            Ok(FhirPathValue::from(value))
        }
    }
        ...
        match self.analyzer.analyze(expression).await {
            Ok(analysis) => {
                if analysis.type_annotations.is_empty() {
                    Ok(format!("No type information available for '{}'", expression))
                } else {
                    let types: Vec<String> = analysis.type_annotations.values()
                        .filter_map(|info| info.fhir_path_type.clone())
                        .collect();
                    if types.is_empty() {
                        Ok(format!("Type information available but no FHIRPath types resolved for '{}'", expression))
                    } else if types.len() == 1 {
            ...
            Err(e) => {
                Ok(format!("Type analysis failed for '{}': {}", expression, e))
            }
        }
    }
    ...
    async fn explain_expression(&self, expression: &str) -> Result<String> {
        ...
        match self.analyzer.analyze(expression).await {
            ...
            Err(e) => {
                Ok(format!("Expression explanation failed: {}", e))
            }
        }
    }
    ...
    async fn get_help(&self, function: Option<&str>) -> Result<String> {
        if let Some(func) = function {
            ...
            if let Some(help) = self.help_system.get_function_help(func) {
                ...
                Ok(format!("Function '{}' is available but detailed help is not yet implemented.\nThis function is registered in the system. Try using it in an expression to see how it works.", func))
            } else {
                Ok(format!("No help available for function '{}'. Try ':help' for a list of available commands.", func))
            }
        } else {
            // General help
    ...
    fn print_welcome(&self) {
        if self.config.color_output {
            ...
            println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            println!("📚 {} Evaluate FHIRPath expressions against FHIR resources", self.bold("Usage:"));
            println!("   • Type expressions: {}, {}, {}", 
            ...
            println!("   • Use commands: {}, {}, {}", 
                ...
                self.blue(":vars"));
            println!();
            println!("🚀 {} Start by loading a FHIR resource: {}", 
                self.bold("Quick Start:"), 
                self.blue(":load <your-file.json>"));
            println!("📖 {} Use {} for available commands and {} for syntax help",
                self.bold("Help:"),
                self.blue(":help"),
                self.blue(":help <operation>"));
            println!("🏃 {} Press {} or use {} to exit",
                self.bold("Exit:"),
            ...
            println!("Help: Use :help for available commands and :help <operation> for syntax help");
...
Examples:
  ...
  :set myVar "hello world""#.to_string()
    }
    ...
    fn start_multiline(&mut self, initial_line: &str) {
        ...
        if self.config.color_output {
            println!("📝 Multi-line mode started. Press Enter on empty line to evaluate, or Ctrl+C to cancel.");
        } else {
            println!("Multi-line mode started. Press Enter on empty line to evaluate, or Ctrl+C to cancel.");
        }
    }
