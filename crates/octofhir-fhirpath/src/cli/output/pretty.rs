...
impl PrettyFormatter {
    ...
    fn success_icon(&self) -> String {
        if self.colored { "✅".to_string() } else { "[OK]".to_string() }
    }

    fn error_icon(&self) -> String {
        if self.colored { "❌".to_string() } else { "[ERROR]".to_string() }
    }

    fn warning_icon(&self) -> String {
        if self.colored { "⚠️".to_string() } else { "[WARNING]".to_string() }
    }

    fn info_icon(&self) -> String {
        if self.colored { "ℹ️".to_string() } else { "[INFO]".to_string() }
    }
}
            ...
            if let Some(ref value) = output.result {
                match value {
                    FhirPathValue::Collection(values) => {
                        result.push_str(&format!("🎯 Results ({} items):\n", 
    ...
    fn format_analysis(&self, output: &AnalysisOutput) -> Result<String, FormatError> {
        {
            ...
            if output.success {
                if let Some(ref analysis) = output.analysis {
                    ...
                    if !analysis.function_calls.is_empty() {
                        ...
                        for func_analysis in &analysis.function_calls {
                            result.push_str(&format!("  🔹 {} ({})\n",
                                ...
                                self.colorize(&func_analysis.signature.description, colored::Color::Yellow)
                            ));
                    ...
                    result.push_str(&format!("{} Analysis complete (no analyzer)\n", self.info_icon()));
                }
            } else if let Some(ref error) = output.error {
