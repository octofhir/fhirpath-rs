...
async fn test_concurrent_evaluation() {
    ...
    println!("Concurrent evaluation results: {} successes, {} errors", success_count, error_count);
    
    // Allow some failures but most should succeed
    assert!(success_count >= 90, "Too many concurrent evaluation failures: {} successes out of 100", success_count);
}
...
async fn test_memory_intensive_operations() {
    ...
    for size in [1_000, 10_000, 50_000] {
        let large_data = json!((0..size).collect::<Vec<i32>>());
        
        // Test operations that process all items
        let result = engine.evaluate("count()", large_data.clone()).await.unwrap();
        assert_eq!(as_single_integer(&result), Some(size as i64));
        
        let result = engine.evaluate("sum()", large_data.clone()).await.unwrap();
        let expected_sum = (size * (size - 1)) / 2; // Sum of 0..size-1
        assert_eq!(as_single_integer(&result), Some(expected_sum as i64));
        
        // Test memory-intensive lambda operations
        let result = engine.evaluate("select($this * 2).sum()", large_data).await.unwrap();
        assert_eq!(as_single_integer(&result), Some((expected_sum * 2) as i64));
        
        println!("Successfully processed collection of size: {}", size);
    }
}
...
async fn test_memory_leak_prevention() {
    ...
    for iteration in 0..100 {
        ...
        for expression in expressions {
            ...
            assert!(result.is_ok(), "Failed on iteration {} with expression {}", iteration, expression);
        }
        ...
        match result {
            Ok(value) => {
                if let Some(expected) = expected_count {
                    assert_eq!(as_single_integer(&value), Some(expected), 
                              "Wrong count for: {}", expression);
                }
                println!("Complex query '{}' took {:?}", expression, duration);
                
                // Should complete in reasonable time
                assert!(duration.as_secs() < 10, "Query too slow: {}", expression);
            }
            Err(e) => {
                println!("Complex query '{}' failed: {}", expression, e);
...
async fn test_rapid_fire_evaluations() {
    ...
    for i in 0..1000 {
        ...
        match engine.evaluate(expression, patient.clone()).await {
            ...
            Err(e) => println!("Evaluation {} failed: {}", i, e),
        }
    }
    ...
    println!("Rapid fire: {} successes in {:?} (avg: {:?})", 
             success_count, total_time, avg_time);
    
    assert!(success_count >= 950, "Too many failures in rapid fire test");
    assert!(avg_time.as_millis() < 10, "Average evaluation time too slow");
}
...
async fn test_mixed_workload_stress() {
    ...
    for i in 0..30 {
        ...
        let patient_clone = patient.clone();
        let handle = task::spawn(async move {
            let result = engine_clone.evaluate("name.where(use='official').given.count()", patient_clone).await;
    ...
    for i in 0..20 {
        ...
        let handle = task::spawn(async move {
            let result = engine_clone.evaluate("where($this > 50).sum()", data).await;
    ...
    println!("Mixed workload results: {} successes, {} failures", successes, failures);
    
    // Should have high success rate even under mixed load
    let success_rate = successes as f64 / (successes + failures) as f64;
    assert!(success_rate >= 0.9, "Mixed workload success rate too low: {:.1}%", success_rate * 100.0);
}
...
async fn test_edge_case_data_handling() {
    ...
    let edge_cases = vec![
        ...
        // Mixed type arrays
        ("mixed_array", json!([1, "string", true, null, {"nested": "object"}, [1, 2, 3]])),
    ...
    for (name, data) in edge_cases {
        ...
        for expression in expressions {
            ...
            match result {
                Ok(value) => {
                    println!("Edge case '{}' with '{}' -> {:?}", name, expression, value);
                }
                Err(e) => {
                    // Some edge cases might fail, which is acceptable
                    println!("Edge case '{}' with '{}' failed: {}", name, expression, e);
                }
            }
        }
    }
