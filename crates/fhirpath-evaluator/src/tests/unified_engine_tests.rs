...
async fn test_all_literal_types() {
    ...
    let result = engine.evaluate("'with\\'escaped\\'quotes'", json!({})).await.unwrap();
    assert_eq!(as_single_string(&result), Some("with'escaped'quotes".to_string()));
...
async fn test_all_binary_operators() {
    ...
    let result = engine.evaluate("false implies true", json!({})).await.unwrap();
    assert_eq!(as_single_boolean(&result), Some(true));
    ...
    let result = engine.evaluate("(1 | 2 | 3).count()", json!({})).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(3));
}
...
async fn test_complex_property_navigation() {
    ...
    // Array indexing
    let result = engine.evaluate("name[0].given[1]", patient.clone()).await.unwrap();
    assert_eq!(as_single_string(&result), Some("Robert".to_string()));
    
    // Property access with filtering
    let result = engine.evaluate("name.where(use = 'official').given", patient.clone()).await.unwrap();
    assert_eq!(count(&result), 2); // "John", "Robert"
    
    // Complex nested access
    let result = engine.evaluate("address[0].line[0]", patient.clone()).await.unwrap();
    assert_eq!(as_single_string(&result), Some("123 Main St".to_string()));
    
    // Access non-existent property
    let result = engine.evaluate("nonexistentProperty", patient.clone()).await.unwrap();
...
async fn test_all_builtin_functions() {
    ...
    // Math functions with literals (independent of input data)
    let result = engine.evaluate("abs(-5)", json!({})).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(5));
    
    let result = engine.evaluate("ceiling(4.3)", json!({})).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(5));
    
    let result = engine.evaluate("floor(4.7)", json!({})).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(4));
    
    // String functions
    let result = engine.evaluate("'hello'.length()", json!({})).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(5));
    ...
    let result = engine.evaluate("allTrue()", bool_data.clone()).await.unwrap();
    assert_eq!(as_single_boolean(&result), Some(false));
    
    let result = engine.evaluate("anyTrue()", bool_data.clone()).await.unwrap();
    assert_eq!(as_single_boolean(&result), Some(true));
}
...
async fn test_error_conditions() {
    ...
    let result = engine.evaluate("nonexistentProperty", json!({"resourceType": "Patient"})).await;
    assert!(result.is_ok());
    assert!(result.unwrap().is_empty());
    
    // Index out of bounds should return empty, not error
    let result = engine.evaluate("[1,2,3][10]", json!({})).await;
    assert!(result.is_ok());
    assert!(result.unwrap().is_empty());
...
async fn test_recursion_depth_limit() {
    ...
    // Create deeply nested expression that will hit recursion limit
    let deep_expr = "1".to_string() + &" + 1".repeat(200); // Very deep binary operations
    let result = engine.evaluate(&deep_expr, json!({})).await;
    
    // Should either error due to recursion depth or succeed if optimized
    match result {
        Ok(_) => {}, // Engine optimized away recursion
        Err(_) => {}, // Hit recursion limit as expected
    }
}
...
async fn test_variable_evaluation() {
    ...
    variables.insert("myVar".to_string(), FhirPathValue::String("test value".into()));
    variables.insert("myNumber".to_string(), FhirPathValue::Integer(42));
    variables.insert("myBool".to_string(), FhirPathValue::Boolean(true));
    
    let result = engine.evaluate_with_variables("%myVar", json!({}), variables.clone()).await.unwrap();
    assert_eq!(as_single_string(&result), Some("test value".to_string()));
    
    let result = engine.evaluate_with_variables("%myNumber", json!({}), variables.clone()).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(42));
    
    let result = engine.evaluate_with_variables("%myBool", json!({}), variables.clone()).await.unwrap();
    assert_eq!(as_single_boolean(&result), Some(true));
    
    // Test undefined variable
    let result = engine.evaluate_with_variables("%undefined", json!({}), variables).await;
    match result {
        Ok(val) => assert!(val.is_empty()),
...
async fn test_parentheses_and_precedence() {
    ...
    let result = engine.evaluate("true or false and false", json!({})).await.unwrap();
...
async fn test_collection_operations() {
    ...
    let data = json!([1, 2, 3, 4, 5]);
    let result = engine.evaluate("sum()", data).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(15));
}
...
async fn test_string_manipulation_functions() {
    ...
    let result = engine.evaluate("'hello world'.contains('world')", json!({})).await;
    if result.is_ok() {
        assert_eq!(as_single_boolean(&result.unwrap()), Some(true));
    }
    
    // Test string starts with
    let result = engine.evaluate("'hello world'.startsWith('hello')", json!({})).await;
    if result.is_ok() {
        assert_eq!(as_single_boolean(&result.unwrap()), Some(true));
    }
    
    // Test string ends with
    let result = engine.evaluate("'hello world'.endsWith('world')", json!({})).await;
    if result.is_ok() {
        assert_eq!(as_single_boolean(&result.unwrap()), Some(true));
    }
    
    // Test substring
    let result = engine.evaluate("'hello world'.substring(6)", json!({})).await;
    if result.is_ok() {
        assert_eq!(as_single_string(&result.unwrap()), Some("world".to_string()));
    }
}
