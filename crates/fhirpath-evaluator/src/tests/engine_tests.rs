...
async fn test_fhir_choice_type_value_access() {
    ...
    let result = engine
        .evaluate("Observation.value", observation_string.clone())
        .await
        .unwrap();
    assert_eq!(result.to_string_value(), Some("test result".to_string()));

    // Direct access should also work
    let result_direct = engine
        .evaluate("Observation.valueString", observation_string)
        .await
        .unwrap();
    assert_eq!(
        result_direct.to_string_value(),
...
async fn test_fhir_choice_type_value_quantity() {
    ...
    let result = engine
        .evaluate("Observation.value.value", observation_quantity.clone())
        .await
        .unwrap();
    ...
    // Direct access should also work
    let result_direct = engine
        .evaluate("Observation.valueQuantity.value", observation_quantity)
        .await
        .unwrap();
...
async fn test_fhir_choice_type_value_boolean() {
    ...
    let result = engine
        .evaluate("Observation.value", observation_boolean.clone())
        .await
        .unwrap();
    ...
    // Direct access should also work
    let result_direct = engine
        .evaluate("Observation.valueBoolean", observation_boolean)
        .await
        .unwrap();
...
async fn test_fhir_choice_type_case_sensitivity() {
    ...
    let result = engine
        .evaluate("TestResource.value", test_resource.clone())
        .await
        .unwrap();
    ...
    let result = engine
        .evaluate("TestResource.value", test_resource_no_direct)
        .await
        .unwrap();
    assert_eq!(
        result.to_string_value(),
        Some("polymorphic_only".to_string())
    );
}
...
async fn test_fhir_choice_type_other_properties() {
    ...
    assert_eq!(
        result.to_string_value(),
        Some("Nizatidine 15 MG/ML Oral Solution".to_string())
    );
...
async fn test_fhir_choice_type_empty_when_not_found() {
    ...
    let result = engine
        .evaluate("Observation.value", observation_no_value)
        .await
        .unwrap();
...
async fn test_fhir_choice_type_collection_handling() {
    ...
    let result = engine
        .evaluate("Bundle.entry.resource.value", bundle)
        .await
        .unwrap();
    ...
    match result {
        octofhir_fhirpath_model::FhirPathValue::Collection(items) => {
            ...
            assert_eq!(
                items.first().unwrap().to_string_value(),
