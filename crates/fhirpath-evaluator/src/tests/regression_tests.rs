...
async fn test_array_indexing_regression() {
    ...
    // Test accessing first element (equivalent to [0])
    let result = engine.evaluate("first()", data.clone()).await.unwrap();
    assert_eq!(as_single_integer(&result), Some(1));

    // Test out-of-bounds access (equivalent to [10])
    let result = engine.evaluate("skip(10).first()", data).await.unwrap();
...
async fn test_nested_function_calls_regression() {
    ...
    if let Some(collection) = as_collection(&result) {
        let counts: Vec<i64> = collection.iter().filter_map(as_single_integer).collect();
