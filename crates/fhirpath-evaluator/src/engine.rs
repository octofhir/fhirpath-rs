...
impl FhirPathEngine {
    ...
    /// Evaluate a string expression against a JSON value (convenience method)
    pub async fn evaluate_str(
        ...
        // Convert JSON to FhirPathValue
        let value = FhirPathValue::from(resource.clone());

        // Evaluate
        self.evaluate(&ast, value).await
    }
    ...
    pub async fn evaluate_async(
        ...
        self.evaluate(expression, input).await
    }
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = EvaluationResult<FhirPathValue>> + 'a>>
    {
        Box::pin(async move {
            match expression {
                ...
                ExpressionNode::Path { base, path } => {
                    let base_val = self.evaluate_expression(base, context).await?;
                ...
                ExpressionNode::Index { base, index } => {
                    let base_val = self.evaluate_expression(base, context).await?;
                    let index_val = self.evaluate_expression(index, context).await?;
                    ...
                    match &base_val {
                        ...
                        _ => Err(EvaluationError::InvalidOperation {
                            message: "Index can only be applied to collections".to_string(),
                        }),
                    }
                }
                ...
                ExpressionNode::Filter { base, condition } => {
                    let base_val = self.evaluate_expression(base, context).await?;
                    let filter_context = context.with_input(base_val.clone());
                    let filter_result =
                        self.evaluate_expression(condition, &filter_context).await?;

                    match self.is_truthy(&filter_result) {
                        true => Ok(base_val),
                        false => Ok(FhirPathValue::Empty),
                    }
                }
                ...
                ExpressionNode::TypeCheck {
                    ...
                    let base_val = self.evaluate_expression(expression, context).await?;
                    // Simple type checking implementation
                    Ok(FhirPathValue::Boolean(
                        self.check_type(&base_val, type_name),
                    ))
                }
                ...
                ExpressionNode::TypeCast {
                    ...
                    let base_val = self.evaluate_expression(expression, context).await?;
                    // Simple type casting - for now, just return the value if it matches
                    if self.check_type(&base_val, type_name) {
                        Ok(base_val)
                    } else {
                        Ok(FhirPathValue::Empty)
                    }
                ...
                ExpressionNode::Lambda(_lambda_data) => {
                    // Lambda expressions require special handling - for now return empty
                    Ok(FhirPathValue::Empty)
                }

                ExpressionNode::Conditional(_conditional_data) => {
                    // Conditional expressions (if-then-else) - for now return empty
                    Ok(FhirPathValue::Empty)
                }
            }
        })
    ...
    fn check_type(&self, value: &FhirPathValue, target_type: &str) -> bool {
        match (value, target_type) {
            ...
            (FhirPathValue::Collection(_), "Collection") => true,
            (FhirPathValue::Resource(_), "Resource") => true,
            _ => false,
        }
    }

    /// Helper method for checking truthiness of a value
    fn is_truthy(&self, value: &FhirPathValue) -> bool {
        match value {
            ...
            _ => true, // Most other values are considered truthy
        }
    }

    /// VM-based evaluation (internal method)
    /// Note: VM integration will be implemented at the higher level (octofhir-fhirpath)
    /// to avoid circular dependencies between compiler and evaluator
    fn try_vm_evaluation(
        &self,
        _expression: &ExpressionNode,
        _input: FhirPathValue,
    ) -> EvaluationResult<FhirPathValue> {
        // VM evaluation will be implemented at the integration layer
        // to avoid circular dependencies between compiler and evaluator crates
        Err(EvaluationError::InvalidOperation {
            message: "VM compilation handled at integration layer".to_string(),
        })
    }
    ...
    pub fn evaluate_with_context_old(
        ...
        // Use futures::executor::block_on to make the async call synchronous
        futures::executor::block_on(self.evaluate_expression(expression, context))
    }
    ...
    pub async fn evaluate_with_context(
        ...
        // Use the new unified evaluation method
        self.evaluate_expression(expression, context).await
    }
    ...
    fn evaluate_literal(&self, literal: &LiteralValue) -> EvaluationResult<FhirPathValue> {
        let value = match literal {
            ...
            LiteralValue::Null => return Ok(FhirPathValue::Empty), // Empty already represents an empty collection
        };

        // Per FHIRPath specification, all results must be collections (even single values)
        Ok(FhirPathValue::singleton(value))
    }
    ...
    fn evaluate_function_call(
        ...
        // Evaluate function
        let result = function
            .evaluate(&unwrapped_args, &registry_context)
            .map_err(EvaluationError::from_function_error)?;
        Ok(result)
    }
    ...
    async fn evaluate_binary_op_async(
        ...
        // Evaluate the operator and wrap result in collection per FHIRPath specification
        let result = operator
        ...
        // Per FHIRPath specification, all results must be collections (even single values)
        match result {
            FhirPathValue::Empty => Ok(FhirPathValue::Empty), // Empty already represents an empty collection
            FhirPathValue::Collection(_) => Ok(result),       // Already a collection
            single_value => Ok(FhirPathValue::singleton(single_value)), // Wrap single values in collections
        }
    }
